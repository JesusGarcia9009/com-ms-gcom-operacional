...
import com.gcom.operacional.service.SourceService;

import lombok.extern.slf4j.Slf4j;

@Slf4j
@RestController
@RequestMapping("${gcom.base-uri}/source")
public class SourceControllerImpl implements SourceController {
     ...
     * 
     * @param SourcesService @see {@link SourcesService}
     */
    public SourceControllerImpl(SourceService sourceService) {
	...
	@Override
	@GetMapping("/list")
	public ResponseEntity<List<ItemDto>> getSourceList() throws IOException {
		log.info(String.format(LOG_START, Thread.currentThread().getStackTrace()[1].getMethodName()));
		List<ItemDto> response = sourceService.findAllSource();
	...
	public ResponseEntity<?> save(@RequestBody ItemDto request) throws Exception {
		...
		try {
			...
			if (e.getMessage() == MSG_SOURCE_DUPL) {
				return new ResponseEntity<String>(MSG_SOURCE_DUPL, HttpStatus.BAD_REQUEST);
			} else {
